...
let as_scalar = function
  ...
  | Tag _|PteVal _ |SysReg _|Instruction _|Frozen _
      -> None
...
let rec compare scalar_compare pteval_compare sysreg_compare instr_compare c1 c2 =
  ...
  | SysReg p1, SysReg p2 -> sysreg_compare p1 p2
  | Instruction i1,Instruction i2 -> instr_compare i1 i2
  | Frozen i1,Frozen i2 -> Int.compare i1 i2
  | (Concrete _,(ConcreteRecord _|ConcreteVector _|Symbolic _|Tag _|PteVal _|SysReg _|Instruction _|Frozen _))
  | (ConcreteVector _,(ConcreteRecord _|Symbolic _|Tag _|PteVal _|SysReg _|Instruction _|Frozen _))
  | (ConcreteRecord _,(Symbolic _|Tag _|PteVal _|SysReg _|Instruction _|Frozen _))
  | (Symbolic _,(Tag _|PteVal _|SysReg _|Instruction _|Frozen _))
  | (Tag _,(PteVal _|SysReg _|Instruction _|Frozen _))
  | (PteVal _,(SysReg _|Instruction _|Frozen _))
  | (SysReg _, (Instruction _|Frozen _))
  | (Instruction _,Frozen _)
    -> -1
  | (Frozen _,(Instruction _|PteVal _|SysReg _|Tag _|Symbolic _|ConcreteRecord _|ConcreteVector _|Concrete _))
  | (Instruction _,(PteVal _|SysReg _|Tag _|Symbolic _|ConcreteRecord _|ConcreteVector _|Concrete _))
  | (SysReg _,(PteVal _|Tag _|Symbolic _|ConcreteRecord _|ConcreteVector _|Concrete _))
...
let rec eq scalar_eq pteval_eq sysreg_eq instr_eq c1 c2 = match c1,c2 with
  ...
  | ConcreteVector v1, ConcreteVector v2 ->
     List.equal (eq scalar_eq pteval_eq sysreg_eq instr_eq) v1 v2
  | ConcreteRecord li1, ConcreteRecord li2 ->
  ...
  | SysReg p1,SysReg p2 -> sysreg_eq p1 p2
  | Instruction i1,Instruction i2 -> instr_eq i1 i2
  | Frozen i1,Frozen i2 -> Misc.int_eq i1 i2
  | (Frozen _,(Instruction _|Symbolic _|Concrete _|ConcreteRecord _|ConcreteVector _|Tag _|PteVal _|SysReg _))
  | (Instruction _,(Symbolic _|Concrete _|ConcreteRecord _|ConcreteVector _|Tag _|PteVal _|SysReg _|Frozen _))
  | (SysReg _,(Symbolic _|Concrete _|ConcreteRecord _|ConcreteVector _|Tag _|PteVal _|Instruction _|Frozen _))
  | (PteVal _,(Symbolic _|Concrete _|ConcreteRecord _|ConcreteVector _|Tag _|SysReg _|Instruction _|Frozen _))
  | (ConcreteRecord _,(ConcreteVector _|Symbolic _|Tag _|Concrete _|PteVal _|SysReg _|Instruction _|Frozen _))
  | (ConcreteVector _,(ConcreteRecord _|Symbolic _|Tag _|Concrete _|PteVal _|SysReg _|Instruction _|Frozen _))
  | (Concrete _,(Symbolic _|Tag _|ConcreteRecord _|ConcreteVector _|PteVal _|SysReg _|Instruction _|Frozen _))
  | (Symbolic _,(Concrete _|Tag _|ConcreteRecord _|ConcreteVector _|PteVal _|SysReg _|Instruction _|Frozen _))
  | (Tag _,(Concrete _|Symbolic _|ConcreteRecord _|ConcreteVector _|PteVal _|SysReg _|Instruction _|Frozen _))
    -> false
...
let normalize s solver =
  ...
  let rec mk_pp pp_symbol pp_scalar pp_label pp_pteval pp_sysreg pp_instr = function
  | Concrete i -> pp_scalar i
  | ConcreteVector vs ->
      let s =
        String.concat ","
          (List.map (mk_pp pp_symbol pp_scalar pp_label pp_pteval pp_sysreg pp_instr) vs)
      in
      sprintf "{%s}" s
  | ConcreteRecord vs ->
      ...
      StringMap.iter
        (fun name c ->
          Printf.bprintf b "%s:%s," name
            (mk_pp pp_symbol pp_scalar pp_label pp_pteval pp_sysreg pp_instr c))
        vs;
      Buffer.add_char b '}';
      Buffer.contents b
  ...
  | SysReg sr -> pp_sysreg sr
  | Instruction i -> pp_instr i
  | Frozen i -> sprintf "S%i" i (* Same as for symbolic values? *)

let pp pp_scalar pp_pteval pp_sysreg pp_instr c =
  let pp_label = sprintf "label:\"P%i:%s\"" in
  let pp = mk_pp pp_symbol pp_scalar pp_label pp_pteval pp_sysreg pp_instr c in
  if _dbg && String.length pp > 6 then "..." else pp

and pp_old pp_scalar pp_pteval pp_sysreg pp_instr =
  let pp_label = sprintf "%i:%s" in
  mk_pp pp_symbol_old pp_scalar pp_label pp_pteval pp_sysreg pp_instr
...
let rec map_scalar f = function
  ...
  | (Symbolic _ | Tag _ | PteVal _ | SysReg _ | Instruction _ | Frozen _) as c ->
      c
...
let rec map_label f = function
  ...
  | (Symbolic _ | Concrete _ | Tag _ | PteVal _ | SysReg _ | Instruction _ | Frozen _) as m
    ->
      m
...
let check_sym v =
  ...
  | Concrete _ | ConcreteVector _ | ConcreteRecord _ | PteVal _ | SysReg _ | Instruction _
  | Frozen _ ->
      assert false
