...
(* Fault handler *)
      ...
      let fault_prop_declare_vars env f =
        fault_prop_iter_vars env (fun t n -> O.fi "%s %s;" t n) f

      let fault_prop_init_vars env f =
        fault_prop_iter_vars env (fun _ n -> O.fi "p->%s = 0;" n) f

      let fault_prop_read_vars env f =
        let rlocs = match f with
          | (_,_,Some prop) -> T.C.rlocations_prop prop
          | _ -> A.RLocSet.empty in
        A.RLocSet.iter
          (fun loc ->
            ...
            | Array (_,sz) ->
               O.fiv
                 ...
                 sz (fault_prop_fmt_var tag f) tag
            | _ ->
               let lhs = sprintf "sf->%s" (fault_prop_fmt_var tag f)
               and rhs =
...
(* Now physical pages in output *)
        ...
        | _::_ ->
           let pp_log_f f =
             O.oi (sprintf "if (p->%s) {" (SkelUtil.dump_fatom_tag A.V.pp_v f)) ;
             begin match f with
             | ((p,lbl),v,Some prop) ->
                let fmt_fault =
                  sprintf "fault(P%d%s,%s," p
                    (match lbl with None -> "" | Some lbl -> ":" ^ lbl) (A.V.pp_v v) in
                EPF.fii ~out:"chan" fmt_fault [];
                let rlocs = T.C.rlocations_prop prop in
                let prefix = sprintf "p->%s" (SkelUtil.dump_fatom_tag (fun _ -> "") f) in
                dump_rlocs prefix rlocs ;
                EPF.fii ~out:"chan" ");" [];
             | _ ->
                ()
             end ;
             O.oi "} else {" ;
             let fmt_fault = sprintf "~%s;" (Fault.pp_fatom A.V.pp_v f) in
             EPF.fii ~out:"chan" fmt_fault [];
             O.oi "}"
           in List.iter pp_log_f faults;
        end ;
        O.o "}" ;
        O.o "" ;
